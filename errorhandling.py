...
class ErrorHandling:
    ...
    def model_file_not_found(self, model_path):
        if not os.path.exists(model_path):
            ...
            print("Download from: https://github.com/ultralytics/assets/releases/download/v8.1.0/yolov8n.pt")
            return False
        return True
    def text_to_speech_error(self, message):
        try:
            ...
            print(f"Voice failed (continuing anyway): {e}")
            return False
    ...
    def dataset_directory_permission_error(self, dataset_dir, filepath, frame):
        try:
            ...
            print(f"Permission denied: Unable to save image to {dataset_dir}. Please check your permissions.")
            return False
        except OSError as e:
            print(f"Could not save image to {dataset_dir}: {e}")
            return False
    ...
    def window_display_error(self, window_name, frame):
        try:
            ...
            print(f"Error displaying window: {e} (running in headless mode?)")
            return False
    def configuration_error(self):   
        valid = True
        if FRAME_WIDTH <= 0 or FRAME_HEIGHT <= 0 or FRAME_WIDTH > 3840 or FRAME_HEIGHT > 2160:
            print("Error: Invalid frame dimensions in configuration. Please check FRAME_WIDTH and FRAME_HEIGHT.")
            valid = False
        if COOLDOWN_TIME < 0:
            print("Error: Invalid cooldown time in configuration. COOLDOWN_TIME must be non-negative.")
            valid = False
        return valid
